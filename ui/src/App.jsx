...
function App() {
  ...
  useEffect(() => {
    fetch('./public/visualization_results/index.json')
      ...
      .then(data => {
        ...
        setYearsMap(data.years);

        // Set default ROI if available
        if (data.regions.length > 0) {
          setSelectedROI(data.regions[0]);
        }
        setLoading(false);
      })
      .catch(err => {
  ...
  // Update years when ROI changes and auto-select logical defaults
  useEffect(() => {
    if (selectedROI && yearsMap[selectedROI]) {
      const availableYears = yearsMap[selectedROI];
      setYears(availableYears);

      // Default logic: Select first year as Before, last year as After
      if (availableYears.length >= 2) {
        setSelectedBeforeYear(availableYears[0]);
        setSelectedAfterYear(availableYears[availableYears.length - 1]);
      } else if (availableYears.length === 1) {
        setSelectedBeforeYear(availableYears[0]);
        setSelectedAfterYear(availableYears[0]);
      } else {
        setSelectedBeforeYear(null);
        setSelectedAfterYear(null);
      }
    }
  }, [selectedROI, yearsMap]);

  const beforeImage = selectedBeforeYear && selectedROI
    ? `./public/visualization_results/${selectedROI}_before_${selectedBeforeYear}.png`
    : null;

  const afterImage = selectedAfterYear && selectedROI
    ? `./public/visualization_results/${selectedROI}_after_${selectedAfterYear}.png`
    : null;
        ...
        {loading ? (
          ...
          <div className="w-full flex flex-col gap-12 animate-slide-up delay-200">
            ...
            <VisualizationViewer
              ...
              maskImage={selectedBeforeYear && selectedROI ? `./public/visualization_results/${selectedROI}_mask_${selectedBeforeYear}.png` : null}
